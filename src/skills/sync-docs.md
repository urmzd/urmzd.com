...
description: "Audit and synchronize project documentation. README, AGENTS.md, llms.txt, docs/, and fsrc markers. Use after feature changes, refactors, or when docs may be stale. Can be run as a scheduled agent or invoked manually."
...
Run `fsrc run --verify` on all files with markers to detect drift:

```sh
# Find all files with fsrc markers
grep -rl 'fsrc src=' . --include='*.md' | xargs fsrc run --verify
```

If any markers are stale, run `fsrc run` to update them:

```sh
grep -rl 'fsrc src=' . --include='*.md' | xargs fsrc run
```
